...
def _entrypoint() -> CallableStrategyEntrypoint:
    return CallableStrategyEntrypoint(
        ...
        _evaluate=lambda ctx: StrategyDecision(
            positions=(PositionTarget(symbol=ctx.market_data["feature_snapshot"][0]["symbol"], target_weight=1.0),),
            diagnostics={"run_as_of": ctx.runtime_config.get("run_as_of")},
        ),
    )


def test_evaluate_feature_snapshot_strategy_fail_closes_when_path_missing() -> None:
    ...
    assert result.decision.risk_flags == ("no_execute",)
    assert result.metadata["snapshot_guard_decision"] == "fail_closed"
    assert result.metadata["fail_reason"] == "feature_snapshot_path_missing"
    assert result.metadata["status_icon"] == "🛑"
    assert result.metadata["managed_symbols"] == ("AAPL", "BOXX")


def test_evaluate_feature_snapshot_strategy_loads_snapshot_into_context() -> None:
    ...
    def snapshot_loader(path: str, **kwargs: Any):
        ...
        return type(
            ...
            (),
            {
                ...
                "metadata": {"snapshot_guard_decision": "proceed", "snapshot_as_of": "2026-04-15"},
            },
        )()
    ...
    assert observed["path"] == "gs://bucket/snapshot.csv"
    assert observed["kwargs"]["run_as_of"] == as_of
    assert observed["kwargs"]["manifest_path"] == "gs://bucket/snapshot.csv.manifest.json"
    assert result.decision.positions[0].symbol == "AAPL"
    assert result.decision.diagnostics["run_as_of"] == as_of
    assert result.metadata["managed_symbols"] == ("AAPL", "BOXX")
    assert result.metadata["status_icon"] == "🧲"
    assert result.metadata["dry_run_only"] is True


def test_evaluate_feature_snapshot_strategy_supports_custom_context_builder() -> None:
    ...
    assert result.decision.positions[0].symbol == "MSFT"


def test_evaluate_feature_snapshot_strategy_can_fail_close_entrypoint_errors() -> None:
    ...
    assert result.decision.risk_flags == ("no_execute",)
    assert result.metadata["snapshot_guard_decision"] == "fail_closed"
    assert result.metadata["fail_reason"] == "feature_snapshot_compute_failed:RuntimeError:boom"


def test_evaluate_feature_snapshot_strategy_raises_entrypoint_errors_by_default() -> None:
    entrypoint = CallableStrategyEntrypoint(
        manifest=StrategyManifest(
            profile="feature_snapshot_strategy",
            domain="us_equity",
