...
每个冻结候选必须显式提供自己的 `ForwardObservationPolicy`：候选 ID、策略 profile、无杠杆基准、前瞻交易日数量、20/60 等复核里程碑，以及恢复前需要的连续健康周期。没有已验证的 P3 历史证据和证据引用时，状态固定为 `PARKED`。
...
达到完整前瞻窗口（SOXL V7 为 252 个交易日）只会进入 `FORWARD_COMPLETE_HUMAN_REVIEW`。返回值永久包含：
