...
2. Add a paper-only platform adapter with a dedicated append-only store and
   audited transition receipts. Crash, duplicate delivery, missing predecessor,
   and input/config mismatch must fail closed.
