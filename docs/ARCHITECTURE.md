# QuantStrategyLab 架构设计规范

> 本文档是 QuantStrategyLab 所有仓库的设计规范。新增、修改平台、策略、插件必须遵守本文档。
...
每个平台服务只有一个权威配置入口 `RUNTIME_TARGET_JSON`。所有引用策略名、平台名的其他配置值都从这里推导，不允许独立设置。

### 3.2 Scheduler 时区 = 市场时区

Scheduler cron 的时区必须等于策略对应的市场时区，不是 Cloud Run 服务部署区域。

### 3.3 策略跨平台兼容

策略在共享包中声明 `compatible_platforms`，平台通过 capability matrix 自动过滤。平台不应硬编码策略列表。

### 3.4 配置自动同步

启动时，config-sync 层自动检查并修正 plugin mounts 和 monitor targets 中的策略名引用，使之与 `RUNTIME_TARGET_JSON.strategy_profile` 对齐。

## 4. 命名规范

| 类型 | 规范 | 示例 |
|---|---|---|
| 平台 ID | 小写+下划线 | `schwab`, `interactive_brokers` |
| 策略 profile | 小写+下划线 | `soxl_soxx_trend_income` |
| 领域 (domain) | 小写+下划线 | `us_equity`, `quant_combo` |
| GitHub 仓库 | 平台用 PascalCase, 策略包用 camelCase | `CharlesSchwabPlatform`, `us-equity-strategies` |
