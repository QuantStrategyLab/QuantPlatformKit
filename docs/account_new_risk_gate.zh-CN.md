...
> 状态：`ADAPTER_SKELETON_IMPLEMENTED_NOT_LIVE_WIRED`

`quant_platform_kit.risk.account_new_risk_gate` 提供账户级「是否禁止新增风险」的
**注入式只读 adapter 骨架**。它与订单级 `RiskEngine` / `risk.gate` 互补，不替代它们。

## 模块边界

| 会做 | 不会做 |
| --- | --- |
| 接受调用方注入的脱敏对账快照投影 | 读取真账户、券商、凭据或网络 |
| 不健康快照 → `NEW_RISK_PROHIBITED` | 自动 reset 熔断 |
| 保持 `live_authority_granted=false` | 部署、改账户启停、授予 live |
| 把 reader/校验异常视为 fail-closed | 削弱或绕过 `RiskEngine` |

健康条件（全部满足才可能 `ALLOW_NEW_RISK`）：

- `observation_status == COMPLETE`
- `reconciliation_status == VERIFIED`
- `circuit_breaker_state == CLOSED`

`ALLOW_NEW_RISK` **不是**下单许可，也不是实盘授权。

## 与 QRT 确定性内核的关系

完整限额/频率判定内核在 QRT `python/scripts/deterministic_risk_gate.py`（仓库 QuantRuntimeSettings）。
本模块先固定「对账健康 → 禁止新增风险」边界与注入点；挂真账户读回、持久化
