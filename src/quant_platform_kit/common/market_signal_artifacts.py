...
from .strategy_plugin_artifacts import download_gcs_object, parse_gcs_uri
...
def materialize_market_signal_artifact_tree(
    ...
    *,
    ...
    _, object_name = parse_gcs_uri(raw_reference)
    local_path = local_path_for_gcs_object(cache_root, object_name)
    return local_path, {
...
def local_path_for_gcs_object(cache_root: Path, object_name: str) -> Path:
    ...
    if raw_object_name.startswith("/"):
        raise ValueError(f"GCS object name must be relative: {raw_object_name}")
    normalized = posixpath.normpath(raw_object_name)
    if normalized in {"", ".", ".."} or normalized.startswith("../"):
        raise ValueError(f"GCS object name escapes the cache root: {raw_object_name}")
    return cache_root.joinpath(*normalized.split("/"))


def resolve_gcs_artifact_reference(base_uri: str, reference: str) -> str:
    raw_reference = _required_string(reference, field_name="reference")
    if raw_reference.startswith("gs://"):
        parse_gcs_uri(raw_reference)
        return raw_reference
    if "://" in raw_reference:
        raise ValueError(f"Unsupported market signal artifact reference: {raw_reference}")
    if raw_reference.startswith("/"):
        raise ValueError(
            "GCS market signal artifacts must use relative linked paths or gs:// URIs: "
            f"{raw_reference}"
        )

    bucket_name, object_name = parse_gcs_uri(base_uri)
    base_dir = posixpath.dirname(object_name)
    resolved = posixpath.normpath(posixpath.join(base_dir, raw_reference))
    if resolved in {"", ".", ".."} or resolved.startswith("../"):
        raise ValueError(
            "GCS market signal artifact reference escapes the bucket root: "
            f"{raw_reference}"
        )
    return f"gs://{bucket_name}/{resolved}"


def _materialize_gcs_json_tree(
    uri: str,
    *,
    cache_root: Path,
    client_factory: Any,
    ...
    local_path = local_path_for_gcs_object(cache_root, object_name)
    download_gcs_object(uri, local_path, client_factory=client_factory)
    visited[uri] = local_path

    payload = _read_json_object(local_path)
    if payload is None:
        return
    for linked_uri in _iter_linked_gcs_artifact_uris(
        payload,
        base_uri=uri,
...
def _iter_linked_gcs_artifact_uris(
    payload: Any,
    *,
    base_uri: str,
    link_fields: frozenset[str],
    ...
    if isinstance(payload, Mapping):
        for key, value in payload.items():
            if key in link_fields and isinstance(value, str) and value.strip():
                yield resolve_gcs_artifact_reference(base_uri, value.strip())
            yield from _iter_linked_gcs_artifact_uris(
                value,
                base_uri=base_uri,
                link_fields=link_fields,
            )
