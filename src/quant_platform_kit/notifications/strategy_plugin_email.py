"""Email publishing helpers for strategy plugin alerts."""
...
class StrategyPluginEmailSettings:
    ...
    def from_object(cls, value: object) -> "StrategyPluginEmailSettings":
        ...
        return cls(
            ...
            sender=_get_value(value, "crisis_alert_email_from"),
            recipients=parse_email_recipients(_get_value(value, "crisis_alert_email_to", ())),
    ...
    def missing_fields(self) -> tuple[str, ...]:
        ...
        if not str(self.sender or "").strip():
            missing.append("CRISIS_ALERT_EMAIL_FROM")
        if not parse_email_recipients(self.recipients):
            missing.append("CRISIS_ALERT_EMAIL_TO")
...
class StrategyPluginEmailAlertMarkerStore:
    ...
    def has_alert(self, alert_key: str) -> bool:
        if self.gcs_prefix_uri:
            return bool(self._gcs_blob(alert_key).exists())
        if self.local_dir:
    ...
    def record_alert(
        ...
        *,
        ...
        if self.gcs_prefix_uri:
            self._gcs_blob(alert_key).upload_from_string(encoded, content_type="application/json")
            return
        if self.local_dir:
    ...
    def _gcs_blob(self, alert_key: str):
    ...
    *,
    ...
    for message in messages:
        ...
        if duplicate:
            ...
            continue
        sent, send_error = _send_message(send_email, message, settings)
        if not sent:
